...
- Python 3.11 recommended (the source uses Python 3.10+ type syntax)
...
- Generate an admin password hash before starting:

```powershell
python -m src.dashboard.auth "your-password"
```

Set the printed value as `ADMIN_PASSWORD_HASH` in `.env`. The insecure
`admin/admin123` fallback is disabled unless `ALLOW_INSECURE_DEFAULT_AUTH=true`
...
- `YOLO_PERSON_MODEL`
  - Person model used by the legacy uniform utility
- `YOLO_HAZARD_MODEL`
  - Path to YOLO hazard weights (default `yolov8m.pt`)
- `FACE_ENCODINGS_PATH`
  - Face encoding artifact path (default `models/face_encodings.pkl`)
...
- `ALLOW_INSECURE_DEFAULT_AUTH`
  - Must be `true` to enable the development-only `admin/admin123` fallback
...
Run the hardware-independent smoke tests from the repository root:
...
These checks cover configuration paths, SQLite initialization and metrics,
authentication, protected dashboard routes, and fallback tracking. They do not
prove that a physical camera, face-recognition native dependency, or YOLO
...
- `processing_ms`
- `active_tracks`
- `camera_status` (`online`, `offline`, or `error`)
...
- Face recognition is disabled with a warning when `face-recognition` is not installed; install it and generate encodings to enable enrollment matching.
- YOLO inference requires the configured weight file and the Ultralytics/PyTorch runtime. Missing or invalid model files are reported as pipeline errors.
- The local camera path requires an available OpenCV-compatible camera. Docker camera passthrough in `docker-compose.yml` is Linux-specific.
- `supervision` enables ByteTrack. Without it, the application uses a small IoU-based fallback that preserves nearby track IDs but is less capable than ByteTrack.
- `src/realtime/face_recognition_live.py` and `src/utils/` contain legacy compatibility code. The supported entrypoint is `python -m src.main`.
