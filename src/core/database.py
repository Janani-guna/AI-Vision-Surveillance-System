...
def init_db():
    ...
    with get_db() as conn:
        ...
        if "active_tracks" not in columns:
            conn.execute("ALTER TABLE metrics ADD COLUMN active_tracks INTEGER DEFAULT 0")
        if "camera_status" not in columns:
            conn.execute("ALTER TABLE metrics ADD COLUMN camera_status TEXT DEFAULT 'unknown'")
        conn.commit()
...
def insert_metrics(
    ...
    with get_db() as conn:
        conn.execute(
                """INSERT INTO metrics
                    (fps, detection_count, face_count, processing_ms, active_tracks, camera_status)
                    VALUES (?,?,?,?,?,?)""",
                (fps, detection_count, face_count, processing_ms, active_tracks, camera_status),
        )
        conn.commit()
