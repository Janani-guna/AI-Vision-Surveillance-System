...
def _get_model() -> Any:
    global _model
    if _model is None:
        if not Config.YOLO_HAZARD_MODEL:
            raise RuntimeError("YOLO_HAZARD_MODEL is not configured")
        import os
        if not os.path.isfile(Config.YOLO_HAZARD_MODEL):
            raise FileNotFoundError(f"YOLO hazard model not found: {Config.YOLO_HAZARD_MODEL}")
        try:
            from ultralytics import YOLO
        except ImportError as exc:
            raise RuntimeError("ultralytics is required for hazard detection") from exc
